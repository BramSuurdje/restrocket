...
export const rateLimiterMiddleware: MiddlewareHandler = async (c, next) => {
	...
	const limiter = await rateLimiter
		...
		.then((limiter) => {
			...
			c.header("X-RateLimit-Limit", String(Number(process.env.RATE_LIMITER_POINTS))); // Using points from rateLimiter config
			c.header("X-RateLimit-Remaining", String(limiter.remainingPoints));
			c.header("X-RateLimit-Reset", new Date(Date.now() + limiter.msBeforeNext).toUTCString());
			return next();
		})
		.catch((limiter) => {
